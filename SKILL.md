...
description: 抖音直播回放下载与短视频二次创作工具。覆盖直播回放 M3U8 下载、视频片段裁剪、竖屏转换(9:16)、慢动作特效(tmix 帧混合)、ASS 字幕烧录、BGM 混音、抖音创作者后台上传前准备。触发条件：用户提到抖音直播回放下载、抖音视频剪辑、短视频二创、游戏直播高光、竖屏视频制作，或者需要在 creator.douyin.com 准备视频草稿时使用。不要替用户点击最终发布按钮。
---

# 抖音视频二创工具包

从直播回放下载到短视频成片，并准备抖音创作者中心上传草稿。
...
- **macOS、Linux shell 或 Windows PowerShell 环境**
...
- 只处理用户拥有版权或明确获得授权的直播回放、视频和音频素材。
- 不要绕过登录、付费墙、访问控制、频率限制或平台反滥用机制。
- 把 M3U8 回放链接视为临时私有访问链接，不要写入公开文档、提交到仓库或在最终回复中完整暴露。
- BGM 默认优先使用抖音平台自己的音乐库或创作者中心可选音乐。只有在用户明确说明已获得授权时，才使用外部音频文件。
- 平台内可用音乐也可能受地区、账号类型或商业化场景限制；发布前提醒用户以平台提示为准。
- 不要移除水印、版权声明或署名标记，除非用户拥有内容且明确要求。
- 不要替用户点击最终「发布」按钮。
...
```
直播回放下载 → 音频分析 → 片段裁剪 → 竖屏转换 → 慢放特效 → 字幕烧录 → BGM混音 → 准备上传草稿 → 用户确认发布
```
...
- 默认优先使用抖音平台自己的音乐库或创作者中心可选音乐
- 如果用户提供本地音频文件，先确认用户拥有使用权或已获得授权
- 不要主动从第三方网站下载音乐
...
如果用户没有提供已授权的本地 BGM，优先在上传草稿后通过抖音创作者中心选择平台内音乐。

## Step 8: 准备抖音发布草稿

xbrowser 自动化只做到发布前准备：
...
5. 按用户要求设置可见性、评论权限等发布选项
6. 等待平台检测完成
7. 停在最终「发布」按钮前，汇报页面状态和待确认信息

**不要替用户点击「发布」按钮。** 最终发布涉及账号行为、内容合规和平台责任，必须由用户亲自确认。
